...
Releases ship thin, usable slices taken from the iterations below. They all serve one goal: **it should be simple to connect your computer to your AI, and to share documents, knowledge and skills with colleagues and friends.** That sharing goes through storage people already use (Google Drive, OneDrive, Dropbox, Box, Nextcloud, a NAS), not git or S3. When the plan runs dry, the next release comes from the GitHub queue.

| Release | Theme | Ships | Issues | Status |
| --- | --- | --- | --- | --- |
| v0.1.0 | Blueprint | Site v2, rebrand, catalog-first site, new artwork | #33 #34 #48 #49 #50 (PRs #45 #51 #52) | shipped 2026.09.10 |
| v0.2.0 | Spaces | `tekt space add/list/sync/remove/autosync`, the `tekt` command, MCPHub `/spaces`, /spaces/ guide, changelog | #53 | shipped 2026.09.10 (#56) |
| v0.3.0 | Connect | `tekt connect`: your Spaces in Claude Code, Claude Desktop and Codex in one command | #54 · #29 · fix #57 | shipped 2026.09.10 |
| v0.4.0 | Shared skills | Skills in a Space appear in everyone's Claude Code after sync; `tekt skill new/list` | #55 · #46 | shipped 2026.09.10 |
| v0.5.0 | Solid ground | Windows winget preflight and honest `[OK]`s, `gh` on every OS | #28 · #26 | shipped 2026.09.10 |
| v0.6.0 | Invite | `tekt space invite` writes the invitation and copies it to the clipboard; `tekt space open`; a plain warning for over-long paths | #61 · #58 | shipped 2026.09.10 |
| v0.7.0 | See your Spaces | `tekt space gui`: RcloneView (freemium) as a point-and-click window onto your storage (Homebrew cask, Windows installer, Linux AppImage) | #31 | shipped 2026.09.10 |
| v0.8.0 | More AI apps | Install Codex CLI, opencode and crush; `tekt connect` reaches opencode (JSON merge, JSONC left alone) and crush (a marked `crushrc` block) | #64 · #29 #36 #37 | shipped 2026.09.10 |
| v0.9.0 | Skill shelf | `tekt skill shelf` and `tekt skill add <skill> [space]`: curated skills (arkitype first) land in a Space and in Claude Code | #46 | shipped 2026.09.10 |
| v0.10.0 | Claude Desktop everywhere | Official Claude Desktop for Linux (beta, Ubuntu/Debian) from Anthropic's apt repo, with a fingerprint-checked key; Windows validated | #24 · #25 | shipped 2026.09.10 |
| v0.11.0 | Tool shelf | `tekt tool shelf / add / remove`: curated MCP servers for every connected app; `tekt tool add memory team` keeps shared AI memory in a Space | #47 | shipped 2026.09.10 |
| v0.12.0 | pi and omp | Install pi and omp (oh-my-pi); shared skills also linked into `~/.agents/skills` for pi | #38 · #75 | shipped 2026.09.10 |
| v0.13.0 | PROFILE | `profile.site` in 00-arkitype is the source; `_data/site.json` generated from it and checked on every build | #39 · arkitype#12 | shipped 2026.09.10 |
| v0.14.0 | Trusted skills | New or changed skills in a Space wait for review before linking (hash-pinned; `_proposed/`; `owner:`); `tekt skill add` installs only from skills.intelcraft (override for Experimental/Avoid). Closes an injection path in shipped `tekt skill link` | #79 · #76 | next |
| v0.15.0 | Space kits | `tekt space add … --kit <type>`: knowledge templates, shelf skills, suggested tools, plain `HARNESS.md`, and a `capture-interview` skill (replaces #42 / #40) | #80 | planned |
| v0.16.0 | Knowledge upkeep | `tekt knowledge lint` (owners, review dates, stale/orphan/conflict files, optional AI pass) and `tekt space report` for managers (supersedes #41 for now) | #81 | planned |
| v0.17.0+ | From the queue | Agent folders inside a Space (#30, narrowed), open-design as a catalog entry (#44), OpenHands (#27), Devin (#32). Research says skip #43 and #41 (owner's call) | | queue |

**Direction (2026.09.10 research, `.manage/2026.09.10.company-harness.research.md`):** agent-orchestration "company harnesses" and self-hosted app deployment are crowded (Paperclip, CrewAI, Coolify, Dokploy). The unclaimed space is knowledge-first: company knowledge captured and kept fresh in storage people already own, trusted skills shared through it, and bring-your-own-AI. The next releases follow that.

The iteration dates below are thematic buckets. The release train sets the actual order.
...
| #46 | Skills shelf: tekt installs hand-curated skills (arkitype plugin first) | 03 |
| #47 | MCP shelf: curation bar + next hand-picked MCP servers | 03 |
